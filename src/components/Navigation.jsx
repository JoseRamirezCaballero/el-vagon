      ...
      <div className='container flex flex-wrap items-center justify-between mx-auto'>
        ...
        <div className={`${isOpen ? 'block' : 'hidden'} items-center justify-between w-full md:flex md:w-auto md:order-1`} id='navbar-cta'>
          <ul className='flex flex-col p-4 mt-4 bg-white md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-900 md:dark:bg-gray-900 dark:border-gray-700'>
            {routes.map((route) => (
              <li key={route.url}>
                <Link href={route.url} className='block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-white dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700'>
                  {route.name}
                </Link>
              </li>
            ))}
