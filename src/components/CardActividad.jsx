...
export default function CardActividad ({ actividad, cantidadEstudiantes }) {
  const porcentaje = Math.round((cantidadEstudiantes / actividad.capacidad_maxima) * 100)
  ...
  const notification = ({ bool, descriptionToast = '' }) => {
    bool
      ? toast.success(`Inscripción ${actividad.periodo} exitosa.`, {
        description: descriptionToast
      })
      : toast.error(`No fue posible inscribirse a: ${actividad.nombre}`, {
        description: descriptionToast
      })
  }
  ...
  const inscribirse = async () => {
    try {
      if (cantidadEstudiantes >= actividad.capacidad_maxima) {
        notification({ bool: false, descriptionToast: 'Se ha alcanzado la capacidad máxima de estudiantes.' })
      ...
      setShowInscripcionModal(false)
      notification({ bool: false, descriptionToast: `Ya estás inscrito en una actividad durante el periodo ${actividad.periodo}, por lo que no es posible registrarte en otra actividad.` })
    }
  }
      ...
      <div className='px-5 pb-5'>
        ...
        <div className='flex items-center mt-2.5 mb-5'>
          <div className='w-full bg-gray-200 rounded-full dark:bg-gray-700'>
            <div style={{ width: porcentajeString }} className='bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full'>
              {porcentajeString}
            </div>
          </div>
          <span className='bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3'>{actividad.creditos}C</span>
        </div>
