...
export default function Cultural ({ actividades }) {
    ...
    <StudentProtectedRoute rol={ROLES.ESTUDIANTE}>
      <PageLayout rol={ROLES.ESTUDIANTE}>
        <div className='flex justify-center mb-2'>
          <h1 className='mb-4 text-3xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-6xl'>Actividades <span className='text-transparent bg-clip-text bg-gradient-to-r to-orange-500 from-amber-400'>Culturales</span></h1>
        </div>
          ...
          {actividades.length > 0
            ...
            : (
              <figure className='max-w-screen-md mx-auto text-center mt-5 p-4 lg:p-0'>
                <p className='text-2xl font-medium text-gray-900 dark:text-white'>Por el momento no hay actividades extraescolares culturales.</p>
...
export async function getServerSideProps () {
  try {
    ...
    const actividades = response.data.filter(
      actividad => actividad.categoria === 'CULTURAL' && actividad.estatus
    )
    return {
      props: { actividades }
    }
